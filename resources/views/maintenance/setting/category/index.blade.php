                        ...
                        <div class="tb-odr-btns d-none d-sm-inline">
                            <a href="#"
                               ...
                               onclick="event.preventDefault();
                                   document.getElementById('delete-customer-form{{ $category->id }}').submit();"
                            >
                                <em class="icon ni ni-trash">
                                </em><span>Supprimer</span>
                            </a>
                            <form id="delete-customer-form{{ $category->id }}"
                                  action="{{ route('maintenance.setting.category.destroy', $category->id) }}" method="post" style="display: none;"
                            >
                                @csrf
                                @method('DELETE')
                            </form>
    ...
    <div class="modal-dialog modal-dialog-top modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title">Saisir une nouvelle categorie</h5>
            </div>
            <div class="modal-body">
                <form id="form-create" action="{{ route('maintenance.setting.category.store') }}" method="post">
                    @csrf
                    <div class="preview-block">
                        <div class="row gy-4">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label class="form-label" for="default-01">Nom de la categorie</label>
...
<script>
    $(document).ready(function() {
        ...
        $('a[data-target$=edit-state]').on('click', function(){
            $.ajax({
                method: "GET",
                url: "{{ route('maintenance.setting.category.edit', ['category' => '#'])  }}".replace('#', $(this).attr('data-resource')),
                dataType: 'json',
                success: function(category){
                    form.attr('action', '{{ route('maintenance.setting.category.update', ['category' => '#']) }}'
                        .replace('#', category.id));
                    $('#edit-state [name=name]').attr('value', category.name);
                },
            })
        });
