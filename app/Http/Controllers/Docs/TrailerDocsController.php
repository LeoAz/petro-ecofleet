...
class TrailerDocsController extends Controller
{
    public function index()
    {
        return view('docs.trailer.index', [
            'trailers' => Trailer::with('documents', 'pattern')
                ->whereHas('documents')
                ->get(),
            'tls' => Trailer::pluck('registration', 'id')
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('docs.trailers.document', $doc->trailer->uuid);
    }

    public function document(Trailer $trailer)
    {
        return view('docs.trailer.docs',[
            'trailer' => $trailer
        ]);
    }
    ...
    private function getAttributes(Request $request): array
    {
        $attributes = $this->validate($request, [
            ...
            'trailer_id.required' => 'L\immatriculation de la remorque est obligatoire',
